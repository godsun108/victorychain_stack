...
def test_stripe_webhook_blocked_in_sovereign_mode_by_default(tmp_path):
    db = _build_db(tmp_path / "stripe-disabled.db")
    request = DummyRequest({"id": "evt_disabled", "type": "payment_intent.succeeded"})
    previous = settings.enable_external_stripe_webhooks
    settings.enable_external_stripe_webhooks = False
    try:
        try:
            asyncio.run(webhooks.stripe_webhook(request, "testsig", db))
            assert False, "expected HTTPException"
        except HTTPException as exc:
            assert exc.status_code == 403
    finally:
        settings.enable_external_stripe_webhooks = previous
        db.close()
