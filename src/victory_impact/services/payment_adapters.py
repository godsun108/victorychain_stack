...
class StripeSDKEventParser:
    ...
    def parse_event(self, payload_bytes: bytes, stripe_signature: str | None):
        if settings.sovereign_mode and not settings.enable_external_stripe_webhooks:
            raise ValueError("stripe webhook ingestion is disabled in sovereign mode")
        if stripe is None:
            return None
...
class InHouseCardBankAdapter:
    ...
    def create_intent(self, payment_method: str, amount: Decimal, currency: str, donor_email: str) -> PaymentIntent:
        ...
        base_url = settings.inhouse_payment_gateway_base_url.rstrip("/")
        return PaymentIntent(
            ...
            currency=currency,
            status="pending",
            checkout_url=f"{base_url}/checkout/{intent_id}?donor={donor_email}",
        )
