...
class EnhancedVictoryChainMCPStrategy(Strategy):
    ...
    def _init_mcp_connection(self):
        ...
        try:
            if EnhancedMCPClient and get_global_mcp_client:
                ...
                logger.warning(
                    "⚠️  Enhanced MCP client not available - using fallback mode"
                )
    ...
    def should_long(self) -> bool:
        ...
        try:
            ...
            # Basic TA conditions
            ta_conditions = [current_rsi < self.rsi_lower, macd_bullish, trend_bullish]
            ...
            if self.use_ai_signals:
                ...
                if signal_data:
                    ...
                    self.ai_signals = signal_data  # Cache for position sizing
            ...
            if self.use_ai_risk_management:
                ...
                if risk_data:
                    ...
                    if (
                        ...
                        or portfolio_heat > self.max_portfolio_heat
                    ):
                        risk_approved = False
            ...
            if should_enter:
                logger.info(
                    f"🟢 LONG ENTRY: TA={ta_score:.2f}, AI={ai_score:.2f}, Combined={combined_score:.2f}"
                )
