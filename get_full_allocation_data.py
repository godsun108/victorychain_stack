...
async def get_full_data():
    ...
    for i, alloc in enumerate(allocations):
        print(
            f"{i+1}. {alloc.symbol}: {alloc.allocation_percentage:.1%} (${alloc.allocation_amount:,.0f}) - {alloc.expected_return:.1%} return"
        )
    ...
    for max_rec in maximizations:
        profit = (
            (
                positions[max_rec.symbol]["current_price"]
                - positions[max_rec.symbol]["entry_price"]
            )
            / positions[max_rec.symbol]["entry_price"]
        ) * 100
        print(
            f"{max_rec.symbol}: {max_rec.recommended_action} (Current: {profit:+.1f}%)"
        )
