#!/usr/bin/env python3
"""
Advanced Data-Driven Market Analysis System
Statistical models, technical indicators, and quantitative analysis
"""
...
class AdvancedDataAnalyst:
    def __init__(self):
        ...
        print("🔬 Loading statistical models and technical indicators...")
    ...
    def calculate_advanced_metrics(
        ...
        metrics["price_momentum"] = {
            "1h_change": (current_price / df_1h["close"].iloc[-2] - 1) * 100,
            "4h_change": (current_price / df_1h["close"].iloc[-5] - 1) * 100,
            "24h_change": (current_price / df_1h["close"].iloc[-25] - 1) * 100,
            "7d_change": (
        ...
        metrics["volatility"] = {
            "hourly_vol": returns_1h.std() * np.sqrt(24) * 100,  # Annualized hourly vol
            "daily_vol": returns_1d.std() * np.sqrt(365) * 100,  # Annualized daily vol
            "vol_of_vol": returns_1h.rolling(24).std().std()
        ...
        metrics["technical_strength"] = {
            ...
            "bb_position": latest_1h["bb_position"],
            "sma_position": 1 if current_price > latest_1h["sma_20"] else -1,
            "volume_strength": latest_1h["volume_ratio"],
        }
        ...
        volume_7d_avg = df_1h["volume"].tail(168).mean() * 24  # 7 days in hours

        metrics["volume_analysis"] = {
            "volume_24h": volume_24h,
            "volume_trend": volume_24h / volume_7d_avg if volume_7d_avg > 0 else 1,
            "volume_consistency": 1
            - df_1h["volume"].tail(24).std() / df_1h["volume"].tail(24).mean(),
            "price_volume_correlation": pearsonr(
                df_1h["close"].tail(24).values, df_1h["volume"].tail(24).values
            )[0],
        }

        # Support/Resistance levels
        highs = df_1h["high"].tail(100)
        lows = df_1h["low"].tail(100)

        metrics["support_resistance"] = {
            "resistance_level": np.percentile(highs, 95),
            "support_level": np.percentile(lows, 5),
            "distance_to_resistance": (np.percentile(highs, 95) - current_price)
            / current_price
            * 100,
            "distance_to_support": (current_price - np.percentile(lows, 5))
            / current_price
            * 100,
        }
    ...
    def perform_statistical_analysis(self, all_metrics: List[Dict]) -> Dict:
        ...
        # PCA analysis
        pca = PCA(n_components=min(3, features_array.shape[1]))
        ...
        return {
            ...
            "cluster_analysis": {
                ...
                "tokens_by_cluster": {
                    i: [symbols[j] for j, c in enumerate(clusters) if c == i]
            ...
            "correlation_insights": {
                "feature_correlations": {
                    f"{feature_names[i]}_{feature_names[j]}": correlation_matrix[i][j]
    ...
    def build_predictive_models(self, all_metrics: List[Dict]) -> Dict:
        ...
        for metrics in all_metrics:
            try:
                ...
                if not any(np.isnan(features)) and not np.isnan(target):
                    X.append(features)
                    y.append(target)
                    symbols.append(metrics["symbol"])
        ...
        for i, symbol in enumerate(symbols):
            ...
            predictions[symbol] = {
                ...
                "confidence": rf_model.score(X_test, y_test),
            }
    ...
    def generate_quantitative_insights(
        ...
        regime = statistical_analysis["statistical_summary"]["market_regime"]
        momentum = statistical_analysis["statistical_summary"]["market_momentum"]
        volatility = statistical_analysis["statistical_summary"]["market_volatility"]

        insights["market_regime_analysis"] = {
            "current_regime": regime,
            "regime_characteristics": self.get_regime_characteristics(regime),
            "momentum_z_score": (momentum - 0)
            / 2,  # Assuming mean=0, std=2 for momentum
            "volatility_percentile": min(volatility / 100, 1.0),
            "regime_stability": statistical_analysis["statistical_summary"][
                "market_trend_strength"
            ],
        }

        # Statistical opportunities (outliers and clusters)
            ...
            insights["statistical_opportunities"].append(
                {
                    ...
                    "reasoning": f"Statistical outlier in {', '.join(outlier['outlier_features'])}",
        ...
        insights["risk_metrics"] = {
            ...
            "correlation_risk": len(
                [
                    c
                    for c in statistical_analysis.get("correlation_insights", {})
                    .get("feature_correlations", {})
                    .values()
                    if abs(c) > 0.8
                ]
            ),
            "outlier_concentration": (
                len(outliers) / len(all_metrics) if all_metrics else 0
            ),
            "regime_uncertainty": 1
            - statistical_analysis["statistical_summary"]["market_trend_strength"],
        }
        ...
        if regime == "bull_trending":
            insights["quantitative_recommendations"].extend(
                [
                    ...
                    "Reduce hedging, increase position sizes",
            ...
            insights["quantitative_recommendations"].extend(
                [
                    ...
                    "Consider short positions on weak technical patterns",
            ...
            insights["quantitative_recommendations"].extend(
                [
                    ...
                    "Increase frequency of position reviews",
                ]
            )
        elif regime == "low_volatility_range":
            insights["quantitative_recommendations"].extend(
                [
                    "Range trading strategies may be effective",
                    "Look for breakout opportunities at range boundaries",
                    "Consider volatility expansion plays",
                ]
            )
    ...
    def run_comprehensive_data_analysis(self) -> Dict:
        ...
        # Sort by volume and take top 30 for detailed analysis
        volume_sorted = sorted(
            usdt_pairs, key=lambda x: float(x["quoteVolume"]), reverse=True
        )
        top_symbols = [t["symbol"] for t in volume_sorted[:30]]

        print(
            f"📊 Analyzing top 30 tokens by volume for comprehensive data analysis..."
        )

        # Get OHLCV data
        ohlcv_data = self.get_comprehensive_ohlcv_data(top_symbols)
        ...
        quantitative_insights = self.generate_quantitative_insights(
            statistical_analysis, predictive_models, all_metrics
        )
        ...
        comprehensive_report = {
            ...
            "raw_metrics": all_metrics[:10],  # Include sample of raw metrics
        }
    ...
    def display_data_driven_results(self, report: Dict):
        ...
        if "model_performance" in report["predictive_models"]:
        ...
        # Feature importance
        if "feature_importance" in report["predictive_models"]:
        ...
        print(f"   Momentum Z-Score: {regime['momentum_z_score']:+.2f}")
        ...
        if stat_opps:
            ...
            for i, opp in enumerate(stat_opps[:5], 1):
                print(
                    f"   {i}. {opp['symbol']}: {opp['reasoning']} (Z-Score: {opp['z_score']:.2f})"
                )
        ...
        if pred_signals:
            ...
            for i, signal in enumerate(pred_signals[:5], 1):
                print(
                    f"   {i}. {signal['symbol']}: {signal['direction'].upper()} "
                    f"({signal['predicted_change']:+.2f}%, Confidence: {signal['model_confidence']:.2f})"
                )

        # Risk metrics
        risk = insights["risk_metrics"]
        print(f"\n⚠️  RISK ASSESSMENT:")
        print(f"   Volatility Regime: {risk['market_volatility_regime'].upper()}")
        print(f"   Correlation Risk: {risk['correlation_risk']} high correlations")
        print(f"   Outlier Concentration: {risk['outlier_concentration']:.1%}")
        print(f"   Regime Uncertainty: {risk['regime_uncertainty']:.1%}")
